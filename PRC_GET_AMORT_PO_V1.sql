CREATE OR REPLACE PROCEDURE MENTARITV_LIVE.PRC_GET_AMORT_PO_V1 (AD_DATE DATE)  IS
/******************************************************************************
         ...
         (
          ( ( ( NVL(C.RUN_AIRED,'N') = 'Y') AND
            ...
            (EXISTS(SELECT 1 FROM RUN_MASTER_RUN_HIST RH
                          ...
                          ( RH.last_date > to_date('20120731','yyyymmdd'))*/)) and
                          ( c.last_date > to_date('20240430','YYYYMMDD')) --Remark Modify - Ridwan, ( c.last_date > '31-May-2009')
         )
         OR ( EXISTS ( select VSALES_CONTRACT_DTL2.ROW_ID_EPI
                      from VSALES_CONTRACT_HDR,
                           VSALES_CONTRACT_DTL,
                           VSALES_CONTRACT_DTL2
                     WHERE VSALES_CONTRACT_DTL.ROW_ID_HDR = VSALES_CONTRACT_HDR.ROW_ID
                       AND  ( VSALES_CONTRACT_HDR.SALES_CONTRACT_DATE  BETWEEN ADD_MONTHS( LD_DATE_FROM, -5)  AND LD_DATE_TO  )  AND
                          ( VSALES_CONTRACT_HDR.SALES_CONTRACT_DATE <= LD_DATE_TO)
                        AND( VSALES_CONTRACT_HDR.SALES_CONTRACT_DATE > to_date('20240430','YYYYMMDD')) --Remark Modify - Ridwan, AND( VSALES_CONTRACT_HDR.SALES_CONTRACT_DATE > '31-Oct-2019')
                       AND ((VSALES_CONTRACT_DTL2.ROW_ID_HDR = VSALES_CONTRACT_DTL.ROW_ID
                      -- AND VSALES_CONTRACT_DTL2.ROW_ID_EPI = B.ROW_ID
                       AND nvl(VSALES_CONTRACT_HDR.APV_STATUS,'N') = 'Y'
                       )
                       )
                       AND B.ROW_ID = VSALES_CONTRACT_DTL2.ROW_ID_EPI
                   )
         )
         ) and
          -- ( H.PO_SUB_GRP IN ('I','O','C') ) AND
          exists ( select 1
                     from INV_SUB_GROUP IVG
                    where IVG.PO_SUB_GRP = H.PO_SUB_GRP
                      AND nvl(IVG.STS_FIN,'N') = 'Y'
          ) AND
         ...
         and (( NVL(a.RUN_AIRED,'N') = 'Y'
          ...
          or (exists ( select 1
                      from VSALES_CONTRACT_HDR,
                           ...
                           VSALES_CONTRACT_DTL2
                     WHERE VSALES_CONTRACT_DTL.ROW_ID_HDR = VSALES_CONTRACT_HDR.ROW_ID
                       AND ( VSALES_CONTRACT_HDR.SALES_CONTRACT_DATE <= LD_DATE_TO)
                        AND( VSALES_CONTRACT_HDR.SALES_CONTRACT_DATE > to_date('20240430','YYYYMMDD')) --Remark Modify - Ridwan, AND( VSALES_CONTRACT_HDR.SALES_CONTRACT_DATE > '31-Oct-2019')
                       AND ((VSALES_CONTRACT_DTL2.ROW_ID_HDR = VSALES_CONTRACT_DTL.ROW_ID
                       AND VSALES_CONTRACT_DTL2.PUR_CONTRACT_NO = x.pur_contract_no
                         and VSALES_CONTRACT_DTL2.ROW_ID_FILM = x.row_id_film )
--                       OR ( EXISTS( SELECT 1
--                                   FROM VSALES_CONTRACT_DTL2_HIST
--                                  WHERE VSALES_CONTRACT_DTL2_HIST.ROW_ID_HDR = VSALES_CONTRACT_DTL.ROW_ID
--                                   AND VSALES_CONTRACT_DTL2_HIST.ROW_ID_EPI = B.ROW_ID )
--                       )
                           )
                        )
              )
       ...
       values
       (x.PUR_CONTRACT_NO ,x.REVISION_NO,x.PUR_TYPE,substr(x.PO_SUB_GRP,-1,1),x.FILM_TITLE,
